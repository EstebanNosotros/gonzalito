            ...
            $(document).on("click", '.btn-edit', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#u_productos_relacionados').val(data.productos_relacionados);
                        $("#u_productos_relacionados-select option[value='"+data.id+"']").remove();
                        oculto = data;
                ...
                <div class="modal-body">
                    <form action="{{ route('productos.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group">
                                <textarea id="descripcion" name="descripcion" rows="4" cols="50" class="form-control @error('descripcion') is-invalid @enderror" placeholder="Resumen de las características del producto">
                                    {{ old('descripcion') }}
                                </textarea>
                                @error('descripcion')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="input-group">
                                ...
                                <select id="productos_relacionados-select" name="productos_relacionados-select" class="form-control @error('productos_relacionados') is-invalid @enderror" value="{{ old("productos_relacionados") }}" multiple required>
                                    <option value="" disabled selected>Seleccione Productos Relacionados</option>
                                    @foreach($data as $producto)
                                        <option value="{{ $producto->id }}">{{ $producto->nombre_web ? $producto->nombre_web : $producto->nombre }}</option>
                                    @endforeach
                                </select>
                    ...
                    <form action="{{ route('productos.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group">
                                ...
                                <select id="u_productos_relacionados-select" name="u_productos_relacionados-select" class="form-control @error('u_productos_relacionados') is-invalid @enderror" multiple required>
                                    <option value="" disabled selected>Seleccione Productos Relacionados</option>
                                    @foreach($data as $producto)
                                        <option value="{{ $producto->id }}">{{ $producto->nombre_web ? $producto->nombre_web : $producto->nombre }}</option>
                                    @endforeach
